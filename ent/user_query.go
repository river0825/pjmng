...
// QueryGroupsa chains the current query on the "groupsa" edge.
func (uq *UserQuery) QueryGroupsa() *GroupQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, user.GroupsaTable, user.GroupsaPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withGroupsa: uq.withGroupsa.Clone(),
...
// WithGroupsa tells the query-builder to eager-load the nodes that are connected to
// the "groupsa" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithGroupsa(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withGroupsa = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGroupsa; query != nil {
		if err := uq.loadGroupsa(ctx, query, nodes,
			func(n *User) { n.Edges.Groupsa = []*Group{} },
			func(n *User, e *Group) { n.Edges.Groupsa = append(n.Edges.Groupsa, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadGroupsa(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.GroupsaTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(user.GroupsaPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.GroupsaPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.GroupsaPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "groupsa" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
