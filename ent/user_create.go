...
func (uc *UserCreate) AddGroupsaIDs(ids ...int) *UserCreate {
	uc.mutation.AddGroupsaIDs(ids...)
	return uc
}

// AddGroupsa adds the "groupsa" edges to the Group entity.
func (uc *UserCreate) AddGroupsa(g ...*Group) *UserCreate {
	ids := make([]int, len(g))
	for i := range g {
		ids[i] = g[i].ID
	}
	return uc.AddGroupsaIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.GroupsaIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
